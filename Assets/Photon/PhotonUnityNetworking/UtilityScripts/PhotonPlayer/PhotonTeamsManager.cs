...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class PhotonTeamsManager : MonoBehaviour, IMatchmakingCallbacks, IInRoomCallbacks
    {
        ...
        void IInRoomCallbacks.OnPlayerLeftRoom(Player player)
        {
            ...
            if (team != null && !playersPerTeam[team.Code].Remove(player))
            {
                Debug.LogWarningFormat("Unexpected situation while removing player {0} who left from team {1}, updating teams for all", player, team);
        ...
        void IInRoomCallbacks.OnPlayerEnteredRoom(Player player)
        {
            ...
            if (playersPerTeam[team.Code].Contains(player))
            {
            ...
            {
                if (playersPerTeam[key].Remove(player))
                {
                    break;
                }
            }
            if (!playersPerTeam[team.Code].Add(player))
            {
                Debug.LogWarningFormat("Unexpected situation while adding player {0} who joined to team {1}, updating teams for all", player, team);
                // revert to 'brute force' in case of unexpected situation
                this.UpdateTeams();
            }
        }
